...
def render(document, review):
    ...
    if not confirmed:
        if not checked['A'] | checked['B']:
            ...
            message = '当前没有已确认的修改点。'
        elif document['limitations']:
            message = '已提取并检查的文字中未发现明确错误；原文件检查范围有限，见范围说明。'
        else:
            message = '未发现明确的文字或标点错误。'
        symbol = '—' if not checked['A'] | checked['B'] or document['limitations'] or review['limitations'] or len(checked['A'] & checked['B']) != len(units) else '✓'
...
def main():
    parser = argparse.ArgumentParser(description="中文文字校对数据校验与离线 HTML 生成，不调用模型或网络。")

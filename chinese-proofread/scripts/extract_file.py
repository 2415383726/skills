...
def write_document(document, target, record, method, overwrite=False, extra_inputs=()):
    ...
    return {'action': 'ready', 'document_path': str(target), 'characters': chars, 'scope': document['scope'],
            'limitations': document['limitations'],
            'suggested_delivery': 'chat' if chars <= policy.CHAT_CHARS and not any(b['kind'] == 'table' for b in document['blocks']) else 'html'}

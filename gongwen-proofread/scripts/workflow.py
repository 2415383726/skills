...
def rules_sha():
    ...
    for name in ('rules.md', 'rule-catalog.json', 'formats/proofread.md',
                 'formats/consistency.md', 'formats/review.md'):
...
def pack_groups(groups, max_candidates):
    packs, current, size = [], [], 0
    for group in groups:
        count = len(group["candidates"])
        if current and size + count > max_candidates:
...
def merge_command(args):
    ...
    consistency = {
        ...
        "document_sha256": manifest["document_sha256"], "required": bool(manifest.get("long_document")),
...
def review_plan_command(args):
    ...
    consistency, consistency_notes = load_consistency(document, manifest["document_sha256"], args.consistency_result, manifest.get("long_document"), sha(merged))
    units = report.document_units(document)
    all_candidates = []
    local_ids = set()
    ...
    outdir = Path(args.output_dir).resolve()
    batches = []
    for selection in pack_groups(candidate_groups(document, combined), args.max_candidates):
        ...
        payload = {
            ...
            "rules_sha256": rules_sha(),
            "context_units": [unit for unit in ordered_units(document) if unit["id"] in context_ids],
            "candidate_groups": review_view(selection),
            "instructions": "先读原文，寻找原句成立的词义与句法解释，再逐组判断修改是否必要。初检理由及来源已隐藏；只读本复核输入。每个候选决定一次，可保留多个不重叠结论。",
        }
...
def finalize_command(args):
    ...
    for batch in plan.get("batches", []):
        ...
        if not path.is_file():
            limitations.append("缺少候选复核结果：" + batch["id"])
    ...
    if merged.get("long_document"):
        method += "跨批次名称检查状态见范围说明。"
...
def main():
    parser = argparse.ArgumentParser(description="公文校对 3.0 文件化工作流，不调用模型或网络。")

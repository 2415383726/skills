...
def prepare_attempt(work, job):
    ...
    metadata['rules_path'] = str(SKILL / 'references' / 'rules.md')
    metadata['format_path'] = str(SKILL / 'references' / 'formats' / (job['kind'] + '.md'))
...
def task_prompt(job):
    role = {'proofread': '公文文字检查 ' + str(job['pass_id']), 'consistency': '跨批次名称一致性检查',
            'review': '候选组复核'}[job['kind']]
    shell, command = submission_command(job)
    return ('角色：{}。\n规则：{}\n结果格式：{}\n原文输入：{}\n结果写入：{}\n'
            '只读以上规则、格式和输入；按格式检查并写结果，不读其他检查结果、不改原文。'
            '写完用 {} 提交：{}\n'
            '字段校验失败时，只修正对应问题后再提交，最多连续3次；不猜定位、不删除真实发现凑通过。'
            '仍失败只报简短失败类别，不回传正文、候选或拦截原话。提交成功即结束，不管理队列、不生成报告。最终成功只回“{} submitted”；失败只回“{} blocked 简短类别”。不加 attempt 编号、结果路径、发现数量或总结。').format(
                role, SKILL / 'references' / 'rules.md', SKILL / 'references' / 'formats' / (job['kind'] + '.md'),
                job['input_path'], job['response_path'] + '.body', shell, command, job['id'], job['id'])
...
def open_job(job_path, context_id=None, context_source=None):
    ...
    with coordinator_lock(work):
        ...
        if context_id is not None and job['kind'] == 'proofread' and job['executor'] != 'main-agent-fallback':
            # Old attempt receipts remain immutable and are included. Serializing
            # opens prevents concurrent A/B claims racing past this comparison.
            for receipt_path in (work / 'jobs').glob('*.json.started'):
                receipt = wf.load(receipt_path)
                if receipt.get('context_id') != context_id:
                    continue
                old_metadata = wf.load(str(receipt_path)[:-len('.started')])
                if (old_metadata.get('kind') == 'proofread'
                        and old_metadata.get('input_path') == job['input_path']
                        and old_metadata.get('pass_id') != job['pass_id']):
                    raise report.DataError('同批 A/B 不能声明同一 context-id；请使用新执行上下文')
        at = now()
